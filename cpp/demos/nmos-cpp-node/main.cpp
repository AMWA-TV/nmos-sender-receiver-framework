...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // Settings can be passed on the command-line, directly or in a configuration file, and a few may be changed
        // dynamically by PATCH to /settings/all on the Settings API
        //
        ...
        // # curl -X PATCH -H "Content-Type: application/json" http://localhost:3209/settings/all -d
        // "{\"logging_level\":-40}" # curl -X PATCH -H "Content-Type: application/json"
...
// Note: the get_ocsp_response callback must be set up before executing the make_node_server where
// make_http_listener_config is set up
        ...
        // cf. preprocessor conditions in nmos::make_node_api, nmos::make_connection_api, nmos::make_events_api,
        // nmos::make_channelmapping_api, make_events_ws_validate_handler
        /*         nmos::experimental::authorization_state authorization_state;
                ...
                {
                    node_implementation
                        .on_validate_authorization(nmos::experimental::make_validate_authorization_handler(node_model,
           authorization_state, nmos::experimental::make_validate_authorization_token_handler(authorization_state,
           gate), gate))
                        .on_ws_validate_authorization(nmos::experimental::make_ws_validate_authorization_handler(node_model,
           authorization_state, nmos::experimental::make_validate_authorization_token_handler(authorization_state,
           gate), gate));
                }
                ...
                {
                    node_implementation
                        .on_get_authorization_bearer_token(nmos::experimental::make_get_authorization_bearer_token_handler(authorization_state,
           gate))
                        .on_load_authorization_clients(nmos::experimental::make_load_authorization_clients_handler(node_model.settings,
           gate))
                        .on_save_authorization_client(nmos::experimental::make_save_authorization_client_handler(node_model.settings,
           gate)) .on_load_rsa_private_keys(nmos::make_load_rsa_private_keys_handler(node_model.settings, gate)) // may
           be omitted, only required for OAuth client which is using Private Key JWT as the requested authentication
           method for the token endpoint
                        .on_request_authorization_code(nmos::experimental::make_request_authorization_code_handler(gate));
           // may be omitted, only required for OAuth client which is using the Authorization Code Flow to obtain the
           access token
                }*/
...
// cf. preprocessor conditions in nmos::make_http_listener_config
        ...
        if(nmos::experimental::fields::server_secure(node_model.settings))
        {
            ...
            node_server.thread_functions.push_back([&, load_ca_certificates, load_server_certificates] {
                nmos::ocsp_behaviour_thread(node_model, ocsp_state, load_ca_certificates, load_server_certificates,
                                            gate);
            });
        }
#endif
        ...
        /* if (nmos::experimental::fields::client_authorization(node_model.settings))
        {
            ...
            if (web::http::oauth2::experimental::grant_types::authorization_code.name ==
        nmos::experimental::fields::authorization_flow(node_model.settings))
            {
                auto load_ca_certificates = node_implementation.load_ca_certificates;
                auto load_rsa_private_keys = node_implementation.load_rsa_private_keys;
                api_routers[{ {}, nmos::experimental::fields::authorization_redirect_port(node_model.settings)
        }].mount({}, nmos::experimental::make_authorization_redirect_api(node_model, authorization_state,
        load_ca_certificates, load_rsa_private_keys, gate));
            }

            // Configure the jwks_uri API (require for Private Key JWK support)

            if (web::http::oauth2::experimental::token_endpoint_auth_methods::private_key_jwt.name ==
        nmos::experimental::fields::token_endpoint_auth_method(node_model.settings))
            {
                auto load_rsa_private_keys = node_implementation.load_rsa_private_keys;
                api_routers[{ {}, nmos::experimental::fields::jwks_uri_port(node_model.settings) }].mount({},
        nmos::experimental::make_jwk_uri_api(node_model, load_rsa_private_keys, gate));
            }

            auto http_config = nmos::make_http_listener_config(node_model.settings,
        node_implementation.load_server_certificates, node_implementation.load_dh_param,
        node_implementation.get_ocsp_response, gate); const auto server_secure =
        nmos::experimental::fields::server_secure(node_model.settings); const auto hsts =
        nmos::experimental::get_hsts(node_model.settings); for (auto& api_router : api_routers)
            {
                auto found = node_server.api_routers.find(api_router.first);

                const auto& host = !api_router.first.first.empty() ? api_router.first.first :
        web::http::experimental::listener::host_wildcard; const auto& port =
        nmos::experimental::server_port(api_router.first.second, node_model.settings);
                ...
                if (node_server.api_routers.end() != found)
                {
                    ...
                    auto listener = std::find_if(node_server.http_listeners.begin(), node_server.http_listeners.end(),
        [&](const web::http::experimental::listener::http_listener& listener) { return listener.uri() == uri; }); if
        (node_server.http_listeners.end() != listener)
                    {
                        found->second.pop_back(); // remove the api_finally_handler which was previously added in the
        make_node_server, the api_finally_handler will be re-inserted in the make_api_listener
                        node_server.http_listeners.erase(listener);
                    }
                    found->second.mount({}, api_router.second);
                    node_server.http_listeners.push_back(nmos::make_api_listener(server_secure, host, port,
        found->second, http_config, hsts, gate));
                }
                else
                {
                    node_server.http_listeners.push_back(nmos::make_api_listener(server_secure, host, port,
        api_router.second, http_config, hsts, gate));
                }
            }
        }
        ...
        // only implement communication with Authorization server if IS-10/BCP-003-02 is required
        /*         if (nmos::experimental::fields::client_authorization(node_model.settings) ||
           nmos::experimental::fields::server_authorization(node_model.settings))
                {
                    ...
                    // and
           https://specs.amwa.tv/is-10/releases/v1.0.0/docs/4.5._Behaviour_-_Resource_Servers.html#public-keys auto
           load_ca_certificates = node_implementation.load_ca_certificates; auto load_rsa_private_keys =
           node_implementation.load_rsa_private_keys; auto load_authorization_clients =
           node_implementation.load_authorization_clients; auto save_authorization_client =
           node_implementation.save_authorization_client; auto request_authorization_code =
           node_implementation.request_authorization_code; node_server.thread_functions.push_back([&,
           load_ca_certificates, load_rsa_private_keys, load_authorization_clients, save_authorization_client,
           request_authorization_code] { nmos::experimental::authorization_behaviour_thread(node_model,
           authorization_state, load_ca_certificates, load_rsa_private_keys, load_authorization_clients,
           save_authorization_client, request_authorization_code, gate); });

                    if (nmos::experimental::fields::server_authorization(node_model.settings))
                    {
                        // When no matching public key for a given access token, it SHOULD attempt to obtain the missing
           public key
           ...
           https://specs.amwa.tv/is-10/releases/v1.0.0/docs/4.5._Behaviour_-_Resource_Servers.html#public-keys
                        node_server.thread_functions.push_back([&, load_ca_certificates] {
           nmos::experimental::authorization_token_issuer_thread(node_model, authorization_state, load_ca_certificates,
           gate); });
                    }
                } */
